...
class _BarChartRace(CommonChart):
    ...
    def podium(self, loc, length, col, cou, ax, i):
        ...
        if self.premiers == []:
            self.premiers.append([name, self.str_index[i]])
        
        elif self.premiers[-1][0] == name and self.str_index[i] != self.premiers[-1][1]:
            self.premiers.append([name, self.str_index[i]])
            self.count += 1
        ...
        ab = AnnotationBbox(im, (leng, loca), xybox=(0, -550), frameon=False, xycoords='data',
                                  boxcoords='offset points', pad=0)
        
        label_text1 = TextArea(name
                               # +list(self.df_origin.loc[self.df_origin["Prénom et nom"].str.contains(name), "Date de naissance"])[0][-10:]
                               +"\n"
                               +"\n"
                               +f"Holds for {self.count} days",
                              {
                             'ha': 'right',
                             'va': 'center',
                             'size': 30,
        ...
        text_ab3 = AnnotationBbox(label_text3, (leng, loca), xybox=(160, -460), frameon=False, xycoords='data',
                                  boxcoords='offset points', pad=0)
        ...
        ab2 = AnnotationBbox(fla, (leng, loca), xybox=(172, -585), frameon=False, xycoords='data',
                                  boxcoords='offset points', pad=0)
        
        
        self.img_label_artist.append(text_ab1)
        ax.add_artist(text_ab1)
        self.img_label_artist.append(text_ab2)
        ax.add_artist(text_ab2)
        self.img_label_artist.append(text_ab3)
        ax.add_artist(text_ab3)
        self.img_label_artist.append(text_ab4)
        ax.add_artist(text_ab4)
        self.img_label_artist.append(ab)
        ax.add_artist(ab)
        self.img_label_artist.append(ab2)
        ax.add_artist(ab2)
    ...
    def _add_tick_label_offset_image(self, location, length, name, ax, i):
        """
        Creates AnnotationBbox objects to display image labels on the graph. There is a 
        new AnnotationBbox object created at each frame. Maybe a better approach is to create
        the original AnnotationBbox objects and then simply update them as the program runs.
        ...
        if self.tick_image_mode=='trailing':     #images move along with the bar
            if self.orientation=='h':
                ...
                # label_text1 = TextArea(f"{age} | {ech} | {''.join([x[0].upper() for x in cat.split(' ')])} | Wins: {wins} | Losses: {losses} | Draws: {draws}",
                #                       {
                #                       # 'ha': 'right',
                #                       # 'va': 'center',
                #                       'size': 15,
                ...
                # ab1 = AnnotationBbox(label_text1, (len_bar,location), xybox=(-280, 0), frameon=False, xycoords='data',
                #                     boxcoords='offset points', pad=0)
                
                
                ab2 = AnnotationBbox(fla, (len_bar,location), xybox=(-38, 0), frameon=False, xycoords='data',
                                    boxcoords='offset points', pad=0, clip_on=True)
        ...
        self.img_label_artist.append(ab)
        ax.add_artist(ab)
        # self.img_label_artist.append(ab1)
        # ax.add_artist(ab1)
        # matplotlib.artist.Artist.set_clip_box(ab1, ax.bbox) 
        # ab1.set_clip_box(ax.bbox)
        self.img_label_artist.append(ab2)
        ax.add_artist(ab2)
    ...
    def prepare_axes(self, ax):
        ...
        ax.grid(True, axis='x', color='#FFD700', ls='-.', linewidth=5, alpha=.7, zorder=0)
        # color='#D3DCE6'

        # ax.tick_params(labelsize=self.tick_label_font['size'], length=0, pad=2)
        ax.tick_params(axis='x', length=12, labelsize=self.tick_label_font['size'], pad=1, color='#ffffff00')
        ...
        # ax.spines['bottom'].set_color('#D3DCE6')
    ...
    def plot_bars(self, ax, i):
        ...
        if self.orientation == 'h':
            ...
            # for index, label in enumerate(labels):
                # labels[index] = label+list(self.df_origin.loc[self.df_origin["Prénom et nom"].str.contains(label), "Date de naissance"])[0][-10:]
                # labels[index] = label+list(self.df_bonus.loc[self.df_bonus["Date"].str.contains(self.str_index[i]), label])[0][3]
            ax.set_yticklabels(ax.get_yticklabels(), **self.tick_label_font, wrap=True)#, visible=False)
            # ax.set_yticklabels(labels, **self.tick_label_font, wrap=True)
            ax.set_xticklabels(ax.get_xticklabels(), **self.tick_label_font, wrap=True)
            #ax.set_yticklabels([])
            ...
            if not self.fixed_max and self.bar_textposition == 'outside':
                ...
                sort = bar_length.copy()
                sort.sort()
                # min_bar = sort[-self.n_bars-7 if len(bar_length)>self.n_bars+7 else 0]
                # min_bar = sort[0]-self.minim
                # print(bar_length)
                ...
                ax.set_xlim(sort[0]-self.minim , new_xmax)
        else:
            ...
            # for index, label in enumerate(labels):
            #     labels[index] = label+list(self.df_bonus.loc[self.df_bonus["Date"].str.contains(self.str_index[i]), label])[0][3]
            # ax.set_xticklabels(labels, **self.tick_label_font)
            if not self.fixed_max and self.bar_textposition == 'outside':
                max_bar = bar_length.max()
                ...
                ax.set_xlim(sort[0]-self.minim, new_ymax)
        ...
        if self.img_label_folder:           #here I am handling the addition of images as the bar tick labels
            ...
            for bar_loc,bar_len,col_name in zipped:
                ...
                self._add_tick_label_offset_image(bar_loc, bar_len, col_name , ax, i)

        self.podium(bar_location, bar_length, cols, colors, ax, i)
        # self.set_background(bar_location, bar_length, new_xmin, new_xmax, ax)
    ...
    def anim_func(self, i):
        ...
        self.progress_bar(i+1, len(self.df_values))
